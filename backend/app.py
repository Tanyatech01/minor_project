...
def signup():
    ...
    if k.fetchone():
        c.close()
        return render_template(import csv
import hashlib
import io
import json
import os
...
# --- 1. BLOCKCHAIN CONFIGURATION (FIXED FOR RENDER) ---
# Use an environment variable for the RPC URL, fallback to local for dev
blockchain_url = os.environ.get("WEB3_PROVIDER_URL", "http://127.0.0.1:8545")
w = Web3(Web3.HTTPProvider(blockchain_url))

try:
    # On Render, accounts[0] won't exist unless you connect to a real RPC
    if w.is_connected():
        w.eth.default_account = w.eth.accounts[0]
except Exception as e:
    print(f"Blockchain connection skipped or failed: {e}")

# Path fix for deployment
base_path = os.path.dirname(__file__)
abi_path = os.path.join(base_path, "../blockchain/build/contracts/s.json")

# Only try to load contract if the file exists (prevents crash on Render)
contract = None
...
a = Flask(__name__)
a.secret_key = os.environ.get("FLASK_SECRET_KEY", "supplychainsecurekey")
o_d = {}

# --- 2. DATABASE CONFIGURATION (FIXED FOR RENDER) ---
def cn():
    # Render provides the full connection string in DATABASE_URL
...
# --- 3. MODEL LOADING ---
d = os.path.dirname(__file__)
# Use try-except to prevent crash if pickle files are missing during first build
try:
    model = pickle.load(open(os.path.join(d, "model.pkl"), "rb"))
    leb = pickle.load(open(os.path.join(d, "le_b.pkl"), "rb"))
    lec = pickle.load(open(os.path.join(d, "le_c.pkl"), "rb"))
except FileNotFoundError:
    print("Warning: AI Model files not found. Prediction features will fail.")
