...
with open("../blockchain/build/contracts/s.json") as f:
    ...
    abi = artifact["abi"]
caddr = w.to_checksum_address("0x36BBff8A7eafeCC8F01C04c3C657Ed250405b7aE")
...
def cn():
    return psycopg2.connect(host="127.0.0.1", dbname="s_chain", user="postgres", password="2118", port="5757")
...
def home():
    ...
    return redirect(url_for("admin") if session["role"] == "admin" else url_for("user_dashboard"))
...
def signup():
    ...
    try:
        k.execute("INSERT INTO users (uname, email, pwd, st) VALUES (%s, %s, %s, %s)", (u, e, p, "Pending"))
        c.commit()
        c.close()
        return render_template("preview.html", msg="Registration Submitted Awaiting Admin Approval")
    except:
        c.close()
    ...
    if k.fetchone()[0] == 0:
        ...
        if os.path.exists(pp):
            with open(pp, "r", encoding="utf-8") as f:
                ...
                for row in r:
                    try:
                        clean_cat = row[4].replace('["', '').replace('"]', '').split(' >> ')[0]
                        k.execute("INSERT INTO p (n, b, pr, cat, s, h, hand, sell, cons) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)", 
                                  (row[3], row[13], row[6], clean_cat, "LEGIT", row[0], "Warehouse Hub", "Retailer", "Active"))
                    except: continue
            c.commit()
            ...
            for row in r:
                if len(row) >= 14:
                    try:
                        ...
                        hh = row[0]
                        k.execute("INSERT INTO p (n, b, pr, cat, s, h, hand, sell, cons) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)", 
                                  (nn, bb, pr, clean_cat, "LEGIT", hh, "Warehouse Hub", "Retailer", "Active"))
                        wr.writerow(row)
                    except: continue
    else:
...
def decommission(pid):
    ...
    if row:
        ...
        new_hash = hashlib.sha256((nn + bb + pr + str(random.random())).encode()).hexdigest()[:16]
        k.execute("INSERT INTO p (n, b, pr, cat, s, h, hand, sell, cons) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)", 
...
def add_u():
    ...
    k = c.cursor()
    try:
        k.execute("INSERT INTO users (uname, email, pwd, st) VALUES (%s, %s, %s, %s)", (u, e, p, "Approved"))
        c.commit()
    except: pass
    c.close()
    return redirect(url_for("admin"))
...
def add_a():
    ...
    k = c.cursor()
    try:
        k.execute("INSERT INTO admins (uname, email, pwd, role) VALUES (%s, %s, %s, %s)", (u, e, p, "Sub-Admin"))
        c.commit()
    except: pass
    c.close()
    return redirect(url_for("admin"))
...
def rst_p():
    if session.get("role") != "admin": return redirect(url_for("home"))
    e, o, p, t = request.form.get("e"), request.form.get("o"), request.form.get("p"), request.form.get("t")
    if o_d.get(e) == o:
        c = cn()
        k = c.cursor()
        if t == "user":
